...
public class TeradataDestinationServiceImpl extends DestinationConnectorGrpc.DestinationConnectorImplBase {
        ...
        private ConfigurationFormResponse getConfigurationForm() {
                ...
                FormField defaultVarcharSize = FormField.newBuilder().setName("default.varchar.size")
                                .setLabel("Default VARCHAR Size")
                                .setRequired(false)
                ...
                FormField varcharCharacterSet = FormField.newBuilder().setName("varchar.character.set")
                                .setLabel("Varchar column CHARACTER SET")
                                .setRequired(false)
                                .setDescription(
                ...
                return ConfigurationFormResponse.newBuilder()
                                ...
                                .addAllFields(Arrays.asList(host, logmech, TD2Logmech, LDAPLogmech, database, tmode,
                                                varcharCharacterSet, defaultVarcharSize, useFastLoad, sslMode,
                                                sslVerifyCa, sslVerifyFull, driverParameters, BatchSize, queryBand))
                                .addAllTests(Arrays.asList(
                                                ConfigurationTest.newBuilder().setName("connect")
                        ...
                        if (TeradataConfiguration.varcharCharacterSet().equals("UNICODE") &&
                                ...
                                try (Connection conn = TeradataJDBCUtil.createConnection(configuration);
                                        ...
                                        Logger.logMessage(Logger.LogLevel.SEVERE,
                                                        String.format("Test failed with exception %s", e.getMessage()));
                                        responseObserver.onNext(TestResponse.newBuilder().setSuccess(false)
        ...
        public void describeTable(DescribeTableRequest request,
                ...
                try {
                        Table t = TeradataJDBCUtil.getTable(conf, database, table, request.getTableName(),
                                        new DescribeTableWarningHandler(responseObserver));
                        Logger.logMessage(Logger.LogLevel.INFO, String.format("Table %s exists",
                                        TeradataJDBCUtil.escapeTable(database, table)));
                        Logger.logMessage(Logger.LogLevel.INFO, String.format("Table %s description: %s",
                                        TeradataJDBCUtil.escapeTable(database, table), t));
                        ...
                        if (e.getCause() instanceof TableNotExistException) {
                                Logger.logMessage(Logger.LogLevel.WARNING, String.format("Table %s doesn't exist",
                                                TeradataJDBCUtil.escapeTable(database, table)));
                                DescribeTableResponse response = DescribeTableResponse.newBuilder().setNotFound(true)
                                                .build();
                                responseObserver.onNext(response);
                        } else {
                                Logger.logMessage(Logger.LogLevel.SEVERE,
                                                String.format("DescribeTable failed for %s with exception %s",
                                                                TeradataJDBCUtil.escapeTable(database, table),
                                                                e.getMessage()));
                                responseObserver.onNext(DescribeTableResponse.newBuilder()
                                                .setWarning(Warning.newBuilder()
                                                                .setMessage("describeTable :: Table: "
                                                                                + TeradataJDBCUtil.escapeTable(database,
                                                                                                table)
                                                                                + ", Error: " + e.getMessage())
                                                                .build())
                                                .build());
        ...
        public void createTable(CreateTableRequest request, StreamObserver<CreateTableResponse> responseObserver) {
                ...
                try (Connection conn = TeradataJDBCUtil.createConnection(conf);
                        ...
                        Logger.logMessage(Logger.LogLevel.SEVERE,
                                        String.format("CreateTable failed with exception %s", e.getMessage()));
                        responseObserver.onNext(CreateTableResponse.newBuilder()
                                        .setTask(Task.newBuilder().setMessage("createTable :: Table: "
                                                        + TeradataJDBCUtil.escapeTable(conf.database(),
                                                                        request.getTable().getName())
                                                        + ", SQL: " + query.replace("\n", " ")
                                                        + ", Error: " + ServiceUtil.getStackTraceOneLine(e)).build())
                                        .build());
                }
                responseObserver.onCompleted();
        }
        ...
        public void alterTable(AlterTableRequest request,
                ...
                try (Connection conn = TeradataJDBCUtil.createConnection(conf);
                        ...
                        query = TeradataJDBCUtil.generateAlterTableQuery(request,
                                        new AlterTableWarningHandler(responseObserver));
                        // query is null when table is not changed
                        if (query != null) {
                                String[] queries = query.split(";");
                                for (String q : queries) {
                                        Logger.logMessage(Logger.LogLevel.INFO,
                                                        String.format("Executing SQL:\n %s", q));
                                        if (!q.trim().isEmpty()) {
                                                stmt.execute(q.trim() + ";");
                                        }
                                }
                        ...
                        Logger.logMessage(Logger.LogLevel.SEVERE,
                                        String.format("AlterTable failed with exception %s", e.getMessage()));
                        responseObserver.onNext(AlterTableResponse.newBuilder()
                                        .setTask(Task.newBuilder()
                                                        .setMessage("alterTable :: Table: "
                                                                        ...
                                                                        + ", SQL: " + query.replace("\n", " ")
                                                                        + ", Error: "
                                                                        + ServiceUtil.getStackTraceOneLine(e))
                                                        .build())
                                        .build());
                        responseObserver.onCompleted();
                }
        ...
        public void truncate(TruncateRequest request,
                ...
                try (Connection conn = TeradataJDBCUtil.createConnection(conf);
                        ...
                        String queryNanosToTimestamp = String.format(
                                        "SELECT TO_TIMESTAMP(CAST('%d' AS BIGINT)) + INTERVAL '0.%06d' SECOND",
                        ...
                        Logger.logMessage(Logger.LogLevel.SEVERE,
                                        String.format("TruncateTable failed for %s with exception %s",
        ...
        public void writeBatch(WriteBatchRequest request,
                ...
                try (Connection conn = TeradataJDBCUtil.createConnection(conf);) {
                        ...
                        ServiceUtil.setTimeZoneToUTCIfNeeded(conn);
                        ...
                        ServiceUtil.processFiles(w, request.getReplaceFilesList(), "No. of files to be written");
                        ...
                        UpdateWriter u = new UpdateWriter(conn, database, table, request.getTable().getColumnsList(),
                                        request.getFileParams(), request.getKeysMap(), conf.batchSize());
                        ServiceUtil.processFiles(u, request.getUpdateFilesList(), "No. of files to be updated");

                        Logger.logMessage(Logger.LogLevel.INFO,
                                        "********************************In DeleteWriter**********************************");
                        DeleteWriter d = new DeleteWriter(conn, database, table, request.getTable().getColumnsList(),
                                        request.getFileParams(), request.getKeysMap(), conf.batchSize());
                        ServiceUtil.processFiles(d, request.getDeleteFilesList(), "No. of files to be deleted");
                        ...
                        Logger.logMessage(Logger.LogLevel.SEVERE,
                                        String.format("WriteBatch failed for %s with exception %s",
                                                        TeradataJDBCUtil.escapeTable(database, table), e.getMessage()));
                        responseObserver.onNext(WriteBatchResponse.newBuilder()
                                        .setTask(Task.newBuilder()
                                                        .setMessage("writeBatch :: Table: "
        ...
        public void writeHistoryBatch(WriteHistoryBatchRequest request,
                ...
                try (Connection conn = TeradataJDBCUtil.createConnection(conf);) {
                        ...
                        Logger.logMessage(Logger.LogLevel.INFO,
                                        "********************************In EarliestStartHistoryWriter**********************************");
                        EarliestStartHistoryWriter e = new EarliestStartHistoryWriter(conn, database, table,
                                        request.getTable().getColumnsList(),
                                        request.getFileParams(), request.getKeysMap(), conf.batchSize());
                        ServiceUtil.processFiles(e, request.getEarliestStartFilesList(),
                                        "No. of files to be written with earliest start");

                        Logger.logMessage(Logger.LogLevel.INFO,
                                        "********************************In UpdateHistoryWriter**********************************");
                        UpdateHistoryWriter u = new UpdateHistoryWriter(conn, database, table,
                                        request.getTable().getColumnsList(),
                                        request.getFileParams(), request.getKeysMap(), conf.batchSize());
                        ServiceUtil.processFiles(u, request.getUpdateFilesList(),
                                        "No. of files to be updated with history");
                        ...
                        w = new LoadDataWriter<>(conf, conn, database, table, request.getTable().getColumnsList(),
                                        ...
                                        conf.useFastLoad(),
                                        new WriteBatchWarningHandler(responseObserver));
                        ServiceUtil.processFiles(w, request.getReplaceFilesList(),
                                        "No. of files to be written with history");
                        ...
                        Logger.logMessage(Logger.LogLevel.INFO,
                                        "********************************In DeleteHistoryWriter**********************************");
                        DeleteHistoryWriter d = new DeleteHistoryWriter(conn, database, table,
                                        request.getTable().getColumnsList(),
                                        request.getFileParams(), request.getKeysMap(), conf.batchSize());
                        ServiceUtil.processFiles(d, request.getDeleteFilesList(),
                                        "No. of files to be deleted with history");
                        ...
                        Logger.logMessage(Logger.LogLevel.SEVERE,
                                        String.format("writeHistoryBatch failed with exception %s", actualMessage));
                        responseObserver.onNext(WriteBatchResponse.newBuilder()
                                        .setTask(Task.newBuilder()
                                                        .setMessage("writeHistoryBatch :: Table: "
                                                                        + TeradataJDBCUtil.escapeTable(database, table)
                                                                        + ", Error: " + actualMessage)
                                                        .build())
                                        .build());
                        ...
                        Logger.logMessage(Logger.LogLevel.SEVERE,
                                        String.format("WriteHistoryBatch failed for %s with exception %s",
                                                        TeradataJDBCUtil.escapeTable(database, table), e.getMessage()));
